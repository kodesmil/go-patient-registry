...
import (
	...
	auth1 "github.com/kodesmil/atlas-app-toolkit/auth"
...
func (m *Profile) ToORM(ctx context.Context) (ProfileORM, error) {
	...
	to.AccountID, err = auth1.GetAccountID(ctx, nil)
	if err != nil {
		return to, err
	}
...
func DefaultPatchSetContact(ctx context.Context, objects []*Contact, updateMasks []*field_mask1.FieldMask, db *gorm1.DB) ([]*Contact, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors1.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}
...
func DefaultApplyFieldMaskContact(ctx context.Context, patchee *Contact, patcher *Contact, updateMask *field_mask1.FieldMask, prefix string, db *gorm1.DB) (*Contact, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedHomeAddress && strings.HasPrefix(f, prefix+"HomeAddress.") {
		...
		if !updatedWorkAddress && strings.HasPrefix(f, prefix+"WorkAddress.") {
		...
		if !updatedNicknames && strings.HasPrefix(f, prefix+"Nicknames") {
			patchee.Nicknames = patcher.Nicknames
			updatedNicknames = true
			continue
		}
	}
	if err != nil {
