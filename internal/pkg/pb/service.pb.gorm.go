...
type ServiceTagORM struct {
	...
	Name      string
	ServiceId *int64
	Services  []*ServiceORM `gorm:"foreignkey:Id;association_foreignkey:Id;many2many:service_tag_services;jointable_foreignkey:service_tag_id;association_jointable_foreignkey:service_id"`
}
...
type ServiceORM struct {
	...
	Tags           []*ServiceTagORM `gorm:"foreignkey:ServiceId;association_foreignkey:Id"`
	Type           int32
	UpdatedAt      *time.Time
}
...
type ServiceSessionORM struct {
	...
	Id          string           `gorm:"type:uuid;primary_key"`
	Offer       *ServiceOfferORM `gorm:"foreignkey:ServiceSessionId;association_foreignkey:Id"`
...
func DefaultStrictUpdateService(ctx context.Context, in *Service, db *gorm1.DB) (*Service, error) {
	...
	filterTags := ServiceTagORM{}
	if ormObj.Id == 0 {
		return nil, errors1.EmptyIdError
	}
	filterTags.ServiceId = new(int64)
	*filterTags.ServiceId = ormObj.Id
	if err = db.Where(filterTags).Delete(ServiceTagORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultStrictUpdateServiceSession(ctx context.Context, in *ServiceSession, db *gorm1.DB) (*ServiceSession, error) {
	...
	*filterClient.ServiceSessionId = ormObj.Id
	...
	filterOffer := ServiceOfferORM{}
	if ormObj.Id == "" {
		return nil, errors1.EmptyIdError
	}
	filterOffer.ServiceSessionId = new(string)
	*filterOffer.ServiceSessionId = ormObj.Id
	if err = db.Where(filterOffer).Delete(ServiceOfferORM{}).Error; err != nil {
		return nil, err
	}
