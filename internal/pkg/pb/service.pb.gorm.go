...
func DefaultListServiceSessionEvaluation(ctx context.Context, db *gorm1.DB) ([]*ServiceSessionEvaluation, error) {
	...
	db, err = gorm2.ApplyCollectionOperators(ctx, db, &ServiceSessionEvaluationORM{}, &ServiceSessionEvaluation{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
...
type ServiceSessionEvaluationORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm1.DB, *[]ServiceSessionEvaluationORM) error
}
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedEvaluation && strings.HasPrefix(f, prefix+"Evaluation.") {
			...
			if patchee.Evaluation == nil {
				patchee.Evaluation = &ServiceSessionEvaluation{}
			}
			if o, err := DefaultApplyFieldMaskServiceSessionEvaluation(ctx, patchee.Evaluation, patcher.Evaluation, &field_mask1.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Evaluation.", db); err != nil {
				return nil, err
			} else {
